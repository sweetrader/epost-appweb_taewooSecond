...
import { fetchCnsmrCntrList } from '@/api/spcCntr'
...
export default {
  ...
  methods: {
    async getList() {
      ...
      const response = await fetchCnsmrCntrList(this.listQuery)
    ...
    goInfo(id) {
      this.$router.push({ name: 'CntrInfoCnsmr', params: { cntrId: id }})
    },
    handleDrawerClose() {
