      ...
      <el-table :data="list" border style="max-width: 1400px !important;">
        <el-table-column :label="$t('spc.sj')" align="left" min-width="150">
          <template slot-scope="scope">
            <span>{{ scope.row.useClNm }}</span> | <span class="edit_button" style="font-size: medium" @click="goEdit(scope.row.rsrcMchId)">{{ scope.row.rentTypeNm + ' ' + scope.row.rent }}</span>
            <span v-if="scope.row.sj !== ''" style="font-size: smaller"><br>{{ scope.row.sj }}  | 면적 {{ scope.row.area }}㎡</span>
            <span style="font-size: smaller"><br>{{ scope.row.addr }} {{ scope.row.addrDtl }}</span>
          </template>
        </el-table-column>
...
import manageForm from './mchManageForm'
import { fetchSuplerMchList } from '@/api/spcMch'
...
export default {
  ...
  methods: {
    async getList() {
      ...
      const response = await fetchSuplerMchList(this.listQuery)
    ...
    goEdit(id) {
      this.$router.push({ name: 'MchInfoSupler', params: { rsrcMchId: id }})
    },
    handleDrawerClose() {
