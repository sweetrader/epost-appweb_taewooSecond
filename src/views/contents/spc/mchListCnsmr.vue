      ...
      <el-table :data="list" border style="max-width: 1400px !important;">
        <el-table-column :label="$t('spc.sj')" align="left" min-width="150">
          <template slot-scope="scope">
            <span>{{ scope.row.useClNm }}</span> | <span class="edit_button" style="font-size: medium" @click="goInfo(scope.row.rsrcMchId)">{{ scope.row.rentTypeNm + ' ' + scope.row.rent }}</span>
...
import { fetchCnsmrMchList } from '@/api/spcMch'
...
export default {
  ...
  methods: {
    async getList() {
      ...
      const response = await fetchCnsmrMchList(this.listQuery)
    ...
    goInfo(id) {
      this.$router.push({ name: 'MchInfoCnsmr', params: { rsrcMchId: id }})
    },
    handleDrawerClose() {
