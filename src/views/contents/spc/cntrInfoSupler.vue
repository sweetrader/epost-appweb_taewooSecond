  ...
  <div v-loading="dataLoading" class="has-head-container">
    ...
    <div class="app-container">
      <el-form ref="ruleForm" :rules="rules" :model="cntr" label-position="left" label-width="90px">
        ...
        <!-- 임대기간 -->
        <el-form-item :label="$t('spc.rentDe')" prop="rentBgnDe">
          <span>{{ cntr.changeRentBgnDe }} ~ {{ cntr.changeRentEndDe }}</span>
        </el-form-item>

        <!-- 메시지 -->
        <el-form-item v-show="cntr.rentState === '04' || cntr.rentState === '06'" :label="$t('spc.mssage')" prop="mssage">
          <el-input v-model="cntr.changeMssage" type="textarea" maxlength="500" :rows="2" readonly/>
        </el-form-item>
        ...
        <el-form-item :label="$t('spc.cntrFirm')" prop="cntrFirm">
          <el-radio-group v-model="cntr.contractId">
    ...
    <div class="drawer-footer">
      <!--거절-->
      <el-button
        v-if="cntr.rentState === '11' || cntr.rentState === '15'"
        v-waves
      ...
      <el-button
        ...
        class="button_style"
        style="width: 55%"
        @click="handleState('12')">
        {{ $t('btn.spc.agree') }}
      </el-button>
      <!--수락-->
      <el-button
        v-if="cntr.rentState === '15'"
        v-waves
        :loading="confirmLoading"
        size="medium"
        type="primary"
    ...
    handleState(cd) {
      ...
      this.$confirm(this.$t('message.editConfirm'), {
        ...
        editCntrState(this.cntr).then(response => {
          ...
          this.$router.push({ name: 'CntrListSupler' })
        }).catch(() => {
          this.confirmLoading = false
        })
      }).catch(() => {
