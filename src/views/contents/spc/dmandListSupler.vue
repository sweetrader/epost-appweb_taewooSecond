      ...
      <el-table :data="list" border style="max-width: 1400px !important;">
        <el-table-column :label="$t('spc.sj')" align="left" min-width="150">
          <template slot-scope="scope">
            <span>{{ scope.row.useClNm }}</span> | <span class="edit_button" style="font-size: small" @click="goInfo(scope.row.dmandCndtnId)">{{ scope.row.sj }}</span>
...
import { fetchSuplerDmandList, editDmandDel } from '@/api/spcDmand'
...
export default {
  ...
  methods: {
    async getList() {
      ...
      const response = await fetchSuplerDmandList(this.listQuery)
    ...
    goInfo(id) {
      this.$router.push({ name: 'DmandInfo', params: { dmandCndtnId: id }})
    },
    handleCreate() {
